 ...
 * @prop {InitFormModel} initFormModel - 必传, formModel的初始值,必须传一个对象
 ...
 * @prop {() => import('vue').MaybeRef<Record<string, any>>} [beforeSaveFormatReqParams] - 通过验证之后, 调用保存接口之前会执行该方法, 此方法的返回值会作为最终请求参数
 */
...
* - 8. beforeSaveFormatReqParams?: 通过验证之后, 调用保存接口之前会执行该方法, 此方法的返回值会作为最终请求参数
*/
export default function useDialog(options) {
  ...
  const onSave = async () => {
    ...
    if (!isPass) return
    await saveState.run(beforeSaveFormatReqParams?.() ?? formModel)
